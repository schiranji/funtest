...
const TimeSlotGenerator = ({setDirty}) => {
              ...
              <StyledAction>
                ...
                <Formik
                  ...
                  initialValues={{
                    ...
                    templateEndDate: endTime === null ? new Date(startTime.getTime() + 15*60000) : endTime,
                    timeSlotGap: 5,
                    timeSlotLength: 30,
                  }}
                  onSubmit={async (newGiftValues, {resetForm}) => {
                    ...
                    const r = await requestBase.post("/grpEvent/generateTimeSlots", newGiftValues, {})
                    const response = r.data;
                    
                    if (response.statusCode === 15) {
                      toaster.warning("More than 100 slots! Check start and end dates and try again.")
                      return
                    }
                    ...
                    {({handleChange, handleBlur, handleSubmit, values: generatorValues, errors, setFieldValue: setInnerFieldValue}) => (
                      <>
                        <Row>
                          <Col>
                            <FormControl label="Start Date" error={errors.templateStartDate}>
                              <Datepicker
                                ...
                                onBlur={handleBlur}></Datepicker>
                            </FormControl>
                          </Col>
                          <Col>
                            <FormControl label="End Date" error={errors.templateEndDate}>
                              <Datepicker
                                formatString="MM/dd/yyyy HH:mm"
                                timeSelectStart
                          ...
                          <Col>
                            <FormControl label="Length of each slot (minutes)" error={errors.timeSlotLength}>
                              <Input
                                name="timeSlotLength"
                                type="number"
                          ...
                          <Col>
                            <FormControl label="Time between slots" error={errors.timeSlotGap}>
                              <Input
                                name="timeSlotGap"
                                type="number"
