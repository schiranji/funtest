...
import {requestBase, getEventDetailUrl, combineDateAndTime, regularNavItems, groupNavItems, getGroupEventDetailUrl, EVENT_TYPE, ATTENDANCE_TYPE, formatTimeToServer, formatTimeFromServer} from '../utils';
...
const Manage = ({eventType}) => {
  ...
  const [editorStateFromEventData, setEditorStateFromEventData] = useState(null)
  ...
  useEffect(() => {
    const loadInEvent = async () => {
      ...
      if (eventType === EVENT_TYPE.regular) {
        getReq = await requestBase.get(`/auth/event/event/view/getEvent/${eventId}`);
        eventMgmtReq = await requestBase.post(`/auth/event/eventManagement/view/getEventManagement/${eventId}`);
        inviteesReq = await requestBase.post(`/auth/event/eventManagement/search/invitees/${eventId}`)
        setInvitees(inviteesReq.data.results);
      } 
      else if (eventType === EVENT_TYPE.group) {
        getReq = await requestBase.get(`/auth/grpEvent/event/edit/searchById/${eventId}`);
        eventMgmtReq = await requestBase.post(`/auth/grpEvent/eventManagement/edit/searchByEventId/${eventId}`);
        inviteesReq = await requestBase.post(`/auth/grpEvent/eventUser/search/getEventUser/${eventId}`)
        setInvitees(inviteesReq.data.results);
      }
      ...
      if (formattedEventData.ticketCategories) {
        formattedEventData.ticketCategories = formattedEventData.ticketCategories.map((t) => {
          if (t.attendeeType) {
      ...
      if (formattedEventData.potluckItems) {
        formattedEventData.potluckItems = formattedEventData.potluckItems.map((t) => {
          if (t.category) {
      ...
      if (formattedEventData.searchTags) {
        formattedEventData.searchTags = formattedEventData.searchTags.map(t => {
          return {id: t, label: t};
        })
      }
      ...
      if (formattedEventMgmtData.eventTeamMembers) {
        formattedEventMgmtData.eventTeamMembers = formattedEventMgmtData.eventTeamMembers.map((t) => {
          if (t.roles) {
      ...
      if (formattedEventMgmtData.eventTasks) {
        formattedEventMgmtData.eventTasks = formattedEventMgmtData.eventTasks.map(t => {
          if (t.memberGroup) {
          ...
          <Row>
            <Col md="3">
              <Navigation 
                ...
                items={eventType === EVENT_TYPE.group ? groupNavItems : regularNavItems}
                overrides={{
                  NavLink: {
                    style: ({$active, $theme}) => {
                      if (!$active) {
            ...
            <Col className="event-preview">
              <Formik
                ...
                onSubmit={async (values) => {
                  const startDateTime = combineDateAndTime(values.startDate, values.startTime).toUTCString()
                  
                  let endDateTime = null;
                  if (values.endDate !== null && values.endTime !== null) {
                    endDateTime = combineDateAndTime(values.endDate, values.endTime).toUTCString();
  
                    if (new Date(endDateTime) <= new Date(startDateTime)) {
                      toaster.negative(<p>Event end should happen after event start.</p>)
                      return
                    }
                  }
                  ...
                  let body = {
                    ...
                    description: draftToHtml(values.summary).trim().replace(/\n/g, "<br>").replace(/\"/g, "'"),
                    ...
                    category2: values.category2 ? values.category2[0].value : null,
                    rsvpRequired: values.rsvpOrTicketed === "rsvp" ? true : false,
                    rsvpMaxCount: values.rsvpOrTicketed === "rsvp" ? values.rsvpMaxCount : 0,
                  ...
                  if (eventData.attendanceMode === ATTENDANCE_TYPE.online) {
                    if (eventData.liveStreamType === "funzippy") { // event was livestream
                  ...
                  let formattedTeamMembers = values.eventTeamMembers.map(t => {
                    let newT = {...t}
                  ...
                  try {
                    if (eventType ===  EVENT_TYPE.group) {
                      await Promise.all([
                        requestBase.post(`/auth/grpEvent/event/edit/updateEvent/${eventData.id}`, JSON.parse(JSON.stringify(body)), {}),
                        requestBase.post(`/auth/grpEvent/eventManagement/edit/updateEventManagement/${eventData.id}`, JSON.parse(JSON.stringify(eventManagementBody)), {}),
                        requestBase.post(`/auth/grpEvent/eventUser/create/invitees/${eventData.id}`, JSON.parse(JSON.stringify(inviteesBody)), {})
                      ])
                      ...
                      await Promise.all([
                        ...
                        requestBase.post(`/auth/event/eventManagement/create/invitees/${eventData.id}`, JSON.parse(JSON.stringify(inviteesBody)), {})
                      ])
                      toaster.positive(<p>Successfully saved changes!</p>)
                    }
                  }
                    ...
                    console.log(e)
                    toaster.negative(<p>Couldn't submit form, please try again.</p>)
                  }
                }}
                initialValues={{
                  ...
                  category2: eventData.category2 ? [{value: eventData.category2}] : null,
                  category3: null,
                  startDate: new Date(formatTimeFromServer(new Date(eventData.startDateTime))),
                  startTime: new Date(formatTimeFromServer(new Date(eventData.startDateTime))),
                  endDate: eventData.endDateTime ? new Date(formatTimeFromServer(new Date(eventData.endDateTime))) : null,
                  endTime: eventData.endDateTime ? new Date(formatTimeFromServer(new Date(eventData.endDateTime))) : null,
                  timeRange: [new Date(eventData.startDateTime),
                  new Date(eventData.endDateTime)],
                  rsvpMaxCount: 1,
                  rsvpOrTicketed: 
                    eventData.rsvpMaxCount > 0 || eventData.rsvpRequired ? "rsvp" 
                    : ((eventData.ticketUrls && eventData.ticketUrls.length > 0) 
                      || (eventData.ticketCategories && eventData.ticketCategories.length > 0) ? "ticketed" : "open"),
                  rsvpRequired: false,
                ...
                {(formikProps) => (
                  <>
                    <EventActions 
                      ...
                      setDirty={setDirty}
                      eventData={eventData}
                      eventType={eventType}></EventActions>
                    ...
                    <TagName 
                      ...
                      formikProps={formikProps}></TagName>
                  </>
                )} 
              </Formik>
            </Col>
